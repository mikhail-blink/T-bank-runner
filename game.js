...
class Boss {
    constructor(level) {
        ...
        this.w = 150; // Increased size slightly? No, keep 150
        this.h = 150;
        this.x = GAME.width - 160; // Pushed right for mobile safety
        this.y = GAME.height - CONFIG.groundY - this.h;
        this.hp = 3 + (level * 2);
        this.maxHp = this.hp;
        this.state = 'idle';
...
window.game = {
    ...
    nextLevel: () => {
        if (GAME.level < 3) {
            ...
            const transTitle = GAME.screens.transition.querySelector('#trans-title');
            const transNext = GAME.screens.transition.querySelector('#trans-next');
...
function loop() {
    ...
    // Resizing (960p for "Zoomed Out" mobile view)
